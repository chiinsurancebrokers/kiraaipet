...
def safe_float(value, default=None):
    """Return a float or default. Prevents Streamlit crashes when session data is blank/None."""
...
def safe_int(value, default=None):
    """Return an int or default. Accepts values saved as strings/floats."""
    if value in (None, "", "None"):
        return default
    try:
...
def render_intake():
    ...
    with c4: weight = st.number_input(t("weight"), min_value=0.0, max_value=150.0,
                                       value=safe_float(pet.get("weight")), placeholder="5.0", format="%.1f")
...
def render_vitals():
    ...
    with tab_vitals:
        ...
        with c1:
            hr   = st.number_input(t("hr"),  min_value=0, max_value=500, value=safe_int(v.get("hr")), placeholder=str(int((hr_range[0]+hr_range[1])//2)))
            temp = st.number_input(t("temp"),min_value=0.0,max_value=45.0,value=safe_float(v.get("temp")), placeholder=str(temp_range[0]), format="%.1f")
        with c2:
            br   = st.number_input(t("br"),  min_value=0, max_value=100, value=safe_int(v.get("br")), placeholder=str(int((br_range[0]+br_range[1])//2)))
            spo2 = st.number_input(t("spo2"),min_value=0, max_value=100, value=safe_int(v.get("spo2")), placeholder="98")
        with c3:
...
else: render_home()    background: white; color: #A7F3D0; position: relative; z-index: 1; }
.kira-step.done   .kira-step-circle { background: #059669; border-color: #059669; color: white; }
.kira-step.active .kira-step-circle { background: #0EA5E9; border-color: #0EA5E9; color: white; box-shadow: 0 0 0 4px rgba(14,165,233,.15); }
.kira-step-label { font-size: 10px; color: #94A3B8; text-align: center; }
.kira-step.done   .kira-step-label { color: #059669; }
